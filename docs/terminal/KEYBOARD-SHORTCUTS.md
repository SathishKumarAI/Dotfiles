...
> Rocky Linux 10.1 | GNOME 47 Wayland | WezTerm | Neovim | Zellij | Rofi

> 🖥️ **Interactive version:** open [`shortcuts.html`](./shortcuts.html) in a browser
> — filterable, categorized, Catppuccin light/dark. This markdown is the plain-text
> source of truth; the HTML mirrors it plus the shell aliases and CLI toolbelt.
>
> **Canonical.** Absorbed the former `docs/md files/KEYBOARD-SHORTCUTS.md` on
> 2026-08-01 — GNOME extensions, Nautilus, TLP, zsh, the Wayland quirks and the
> memorisation order came from there. There is no other keybindings reference.
...
## Neovim — Leader = Space (Lazy + Snacks/Oil/Neogit)
...
| Shortcut | Action |
|----------|--------|
| `-` | Open parent dir — edit filesystem as a buffer |
...
| `Space` (alone) | which-key popup of all leader maps |
